...
GlesPlay::GlesPlay(JNIEnv *env, jobject thiz, PlayCallback *playCallback, const char *dataSource,
                   jobject surface) {
     ...
     *
     * 解决上面的坑， 自己copy才行
     * 【strlen[dataSource]】 这段代码有坑： 因为（hello 在C++中是 hello\n ） 所以需要加1
     */
...
GLint initShader(const char *source, int type) {
    ...
    //加载shader 参数1代表： shader 数量；  参数0代表： 代码长度， 传0则读到字符串结尾
...
void GlesPlay::playYUV(jobject surface) {
    ...
    //3.2 创建surface（egl和NativeWindow 进行关联。 最后一个参数为属性信息， 0表示默认版本
    ...
    GLint fsh = initShader(fragYUV420P, GL_VERTEX_SHADER);
    ...
    FILE *fp = fopen(dataSource, "rb");
    if (!fp) {
        LOGD("oepn file %s fail", dataSource);
        return;
    }
...
void GlesPlay::showMessage(JNIEnv *env, const char *msg, bool b) {
    if (this->playCallback) {
         if (b) {
             this->playCallback->onSucceed(msg);
         } else {
             this->playCallback->onError(msg);
         }
    }
}
